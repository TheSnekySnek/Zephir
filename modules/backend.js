    ...
    socket.on('walkCoins', function(msg){
        ...
        if(User){
            ...
            if(userID){
                ...
                client.guilds.get(DB.getBotData().guild).channels.get('228046069735489536').send(client.guilds.get(DB.getBotData().guild).members.get(userID) + " just finished walking " + msg.meters + " meters.")
            }
        }
        socket.emit('disco')
    })
